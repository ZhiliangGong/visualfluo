...
handles = figure('Visible','off','Name','on','NumberTitle','off','Units','pixels',...
    'Position',[190,15,1200,800],'Resize','on');
...
%% upper plot
ax1 = axes('Parent',handles,'Units','normalized','Position',[0.215 0.5 0.45 0.46]);
ax1.XLim = [0 10];
ax1.YLim = [0 10];
...
%% lower plot
ax2 = axes('Parent',handles,'Units','normalized','Position',[0.215 0.05 0.45 0.38]);
ax2.XLim = [0 10];
ax2.YLim = [0 10];
...
uicontrol(rightPanel,'Style','text','String','Detector Footprint (mm):','HorizontalAlignment','left','Units','normalized',...
    'Position',[0.03 0.9 0.35 0.022]);
detectorFootprint = uicontrol(rightPanel,'Style','edit','String','10.76','HorizontalAlignment','left','Units','normalized',...
    'Position',[0.35 0.901 0.12 0.022],'CallBack',@adjustFoorprintCalibration);

uicontrol(rightPanel,'Style','text','String','Scaling x:','HorizontalAlignment','left','Units','normalized',...
    'Position',[0.51 0.9 0.15 0.022]);
scaleFactor = uicontrol(rightPanel,'Style','edit','String','1000000','HorizontalAlignment','left','Units','normalized',...
    'Position',[0.65 0.901 0.22 0.022],'CallBack',@scaleFactor_Callback);
...
calibration1 = uicontrol(rightPanel,'Style','edit','String','','HorizontalAlignment','left','Units','normalized',...
    'Position',[0.24 0.871 0.13 0.022],'CallBack',@adjustFoorprintCalibration);
...
calibration2 = uicontrol(rightPanel,'Style','edit','String','','HorizontalAlignment','left','Units','normalized',...
    'Position',[0.39 0.871 0.13 0.022],'CallBack',@adjustFoorprintCalibration);
...
calibration3 = uicontrol(rightPanel,'Style','edit','String','','HorizontalAlignment','left','Units','normalized',...
    'Position',[0.65 0.871 0.13 0.022],'CallBack',@adjustFoorprintCalibration);
...
%% callback functions
    ...
    function refreshButton_Callback(source,eventdata) %refresh data without losing current work
        ...
        if ~isempty(x)
            ...
            if length(newScanNumbers) > length(oldScanNumbers)
                ...
                footprint = str2double(detectorFootprint.String);
                A = str2double(scaleFactor.String);
                y = importData(info.specFile,info.specPath,scanFiles,info.scanPath,A,footprint);
    ...
    function adjustFoorprintCalibration(source,eventdata) %change the detector footprint for the current and following scans
        ...
        for i = scanList.Value(1):length(x)
            x{i}.footprint = footprint;
            x{i}.calibration = calibration;
            adjustFootprintScaleFactorCalibration(x{i});
        end
        displayScanInfo;
        upperPlot;
        lowerPlot;
...
%% functions that make changes to the UI
    ...
    function displayScanInfo %display the important information
        ...
        %the input boxes
        detectorFootprint.String = num2str(x{scanList.Value(1)}.footprint);
...
%% data import
    ...
    function flag = loadDataAt(thePath,specFile) %try to load data, returns 0 if not successufl, 1 if successful
        ...
        if ~isempty(specFile)
            ...
            if ~isempty(scanFiles)
                ...
                A = str2double(scaleFactor.String);
                try
                    x = importData(specFile,specPath,scanFiles,scanPath,A,footprint);
    ...
    function y = importData(specFile,specPath,scanFiles,scanPath,A,footprint) %return a cell array
        
        scanNumbers = getScanNumbers(scanFiles);
        
        y = cell(1,length(scanNumbers));
        for n = 1:length(scanNumbers)
            y{n} = VisualFluoData(specFile,specPath,scanFiles{n},scanPath,scanNumbers(n),A,footprint);
        end
